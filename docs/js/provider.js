...
const getProvider = async () => {
  try {
    if (typeof web3 !== 'undefined') {
      ...
      console.log("local server connect");
...
const login = async () => {
  try {
    ...
    const accountNumber = our_server_accountNumber;
    try {
      // const accounts = await web3MetamaskProvider.eth.getAccounts()
      const accounts = await web3LocalhostProvider.eth.getAccounts()
    ...
    try {
      ...
      metaMaskUserObject.localAccount = localAccounts[accountNumber]
      localStorage.setItem('metaMaskUserObject', JSON.stringify(metaMaskUserObject));
      console.log(metaMaskUserObject);
...
const clear = ()=>{
  ...
  console.log("remove web3MetamaskProvider");
